import type { ContaVencidaDetalheItem } from '@/types/dashboard'
import { cn } from '@/lib/utils'

const fmt = (v: number) => v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })

interface Props { dados: ContaVencidaDetalheItem[] }

export default function TabelaContasVencidas({ dados }: Props) {
  return (
    <div className="rounded-lg border bg-card overflow-hidden">
      <div className="px-4 py-3 border-b bg-muted/30 flex items-center justify-between">
        <p className="text-sm font-semibold">Contas Vencidas</p>
        {dados?.length > 0 && (
          <span className="text-xs bg-red-100 text-red-700 dark:bg-red-950/30 dark:text-red-400 px-2 py-0.5 rounded-full font-medium">
            {dados.length} pendente{dados.length !== 1 ? 's' : ''}
          </span>
        )}
      </div>
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          {!dados?.length && (
            <tr><td colSpan={4} className="px-4 py-6 text-center text-muted-foreground">Nenhuma conta vencida</td></tr>
          )}
        </tbody>
      </table>
    </div>
