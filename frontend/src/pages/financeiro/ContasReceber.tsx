...
export default function ContasReceber() {
  ...
  useEffect(() => {
    reload()
    void listCategorias('Receita').then(cs => setCategorias(cs.map(c => c.name).sort())).catch(() => {})
  ...
  const total = allFiltered.reduce((s, l) => s + l.amount, 0)
  const totalVencido = allFiltered.filter(l => l.vencido).reduce((s, l) => s + l.amount, 0)
  const totalAVencer = allFiltered.filter(l => !l.vencido).reduce((s, l) => s + l.amount, 0)
              ...
              return (
                <div key={l.id} className="rounded-lg border bg-card p-4 space-y-2">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold text-primary">{fmt(l.amount)}</span>
                  </div>
                        ...
                        <div key={l.id} className={`p-3 pl-6 space-y-1 ${l.vencido ? 'bg-destructive/5' : ''}`}>
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <span className="text-sm font-semibold text-primary">{fmt(l.amount)}</span>
